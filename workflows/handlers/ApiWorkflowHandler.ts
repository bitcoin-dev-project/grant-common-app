...
export class ApiWorkflowHandler implements WorkflowHandler {
  ...
  async submit(application: Record<string, unknown>, org: Organization): Promise<SubmissionResponse> {
    try {
      ...
      await this.sendTeamCopy(mappedApplication, org);
      
      return {
        success: true,
        message: `Application submitted successfully to ${org.name}${process.env.BDP_TEAM_EMAIL ? ' and team copy sent' : ''}`,
        data: response.data
      };
    } catch (error: unknown) {
  ...
  private async sendTeamCopy(
    ...
    try {
      ...
      console.log(`📧 Sending team copy for ${org.name} API submission to: ${teamEmail}`);
      
      // Send team copy using EmailWorkflowHandler
      await emailHandler.submit(application, teamOrg);
      
      console.log(`✅ Team copy sent successfully for ${org.name} API submission`);
    } catch (error) {
      // Log error but don't fail the whole submission
      console.error(`❌ Error sending team copy for ${org.name} API submission:`, error);
    }
  }
} 